...
import { isValidSessionId } from "@/lib/realtime/viewer-socket";
...
export default async function LiveSessionPage({
  ...
  return <LiveSessionClient key={sessionId} sessionId={sessionId} />;
}
...
function InvalidSession() {
  return (
    <div className="min-h-screen bg-background text-text-primary">
      <LiveHeader />
      <main className="mx-auto flex w-full max-w-[1280px] flex-1 items-center justify-center px-4 py-16 sm:px-8 lg:px-12">
        <section className="w-full max-w-xl rounded-2xl border border-red-200 bg-white p-8 text-center shadow-[0_10px_30px_-16px_rgba(17,24,39,0.16)] sm:p-12">
          ...
          <h1 className="mt-3 font-display text-2xl font-semibold text-text-primary sm:text-3xl">
            This live session ID isn’t valid
          </h1>
          <p className="mt-3 text-sm leading-6 text-text-secondary sm:text-base">
            Check the link you received. Session IDs may contain letters,
            numbers, periods, underscores, and hyphens, up to 64 characters.
          </p>
        </section>
      </main>
    </div>
